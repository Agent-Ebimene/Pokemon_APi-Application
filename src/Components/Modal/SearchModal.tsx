...
const SearchModal = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const res = pokemons.filter(obj => Object.values(obj).some(val => val.includes(e.target.value)));
    setResult(res)
  }
  ...
  return (
    <div
      ...
      id="staticBackdrop"
      data-bs-backdrop={`static ${!modalOpen && false}`}
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-body">
            ...
            {result.map((pokemon: PokemonItem) => <h3 onClick={navigateHandler(pokemon.name)} key={pokemon.name}>{pokemon.name}</h3>)}
          </div>
        </div>
      </div>
    </div>
